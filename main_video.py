...
def get_args_parser():
    parser = argparse.ArgumentParser('PAdapter: Adapter Combined with Prompt for Image and Video Classification', add_help=False)
    ...
    parser.add_argument('--fulltune', default=True, action='store_true', help='full finetune model')
    parser.add_argument('--inception', default=False, action='store_true', help='whether use INCPETION mean and std for Jx provided IN-21K pretrain')

    # PAdater related parameters
    parser.add_argument('--padapt_on', default=0, type=int, help='whether activate PA')
    parser.add_argument('--padapt_L_size', default=784, type=int, help='number of bottleneck of PA')
    parser.add_argument('--padapt_learn_mode', default='add', type=str, help='how to learn prompt of PA', choices=['cat', 'add'])
    parser.add_argument('--padapt_cat_mode', default='x_p', type=str, help='how to cat prompt with feature in PA', choices=['x_p', 'p_x_p', 'p_2x_p'])
    parser.add_argument('--padapt_local', default='a_m_p', type=str, help='where to use PA', choices=['p_a_m', 'a_p_m', 'a_m_p', 'p+a_m', 'a_p+m', 'p+b'])
    parser.add_argument('--padapt_scalar', default='0.', type=str, help='setting of scale')
    parser.add_argument('--padapt_bottleneck', default=32, type=int, help='bottleneck of PA')
    ...
    parser.add_argument('--dist_eval', action='store_true', default=True, help='Enabling distributed evaluation (recommended during training for faster monitor')
    ...
    parser.add_argument('--drop_path', type=float, default=0.0, metavar='PCT', help='No drop path for linear probe')
...
def main(args):
    ...
    tuning_config = EasyDict(
        ...
        # PAdapter related
        padapt_on=args.padapt_on,
        padapt_L_size=args.padapt_L_size,
        padapt_learn_mode=args.padapt_learn_mode,
        padapt_cat_mode=args.padapt_cat_mode,
    ...
    if args.finetune and not args.eval:
        ...
        if os.path.basename(args.finetune).startswith('pretrain'):
            ...
            del checkpoint_model['norm.weight']
            del checkpoint_model['norm.bias']
        elif os.path.basename(args.finetune).startswith('swin'):
            checkpoint_model = OrderedDict()
            for k, v in raw_checkpoint_model['state_dict'].items():
                if k.startswith('backbone.'):
                    checkpoint_model[k[9:]] = v
        elif os.path.basename(args.finetune).startswith('videomae'):
            checkpoint_model = OrderedDict()
            for k, v in raw_checkpoint_model.items():
                if k.startswith('encoder.'):
                    checkpoint_model[k[8:]] = v  # remove 'encoder.' prefix
            del checkpoint_model['norm.weight']
            del checkpoint_model['norm.bias']
        elif os.path.basename(args.finetune).startswith('r'):
            checkpoint_model = OrderedDict()
            for k, v in raw_checkpoint_model['state_dict'].items():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir and test_stats["acc1"] > max_accuracy:
            ...
            log_model = args.output_dir + '/model.txt'
            with open(log_model, 'w') as f:
                f.write(str(model))
